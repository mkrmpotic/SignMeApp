...
public class SwipeToDismissTouchListener<SomeCollectionView extends ViewAdapter>
	...
	public class RowContainer {
		...
		View getCurrentSwipingView() {
			return dataContainerHasBeenDismissed ? undoContainer
					: dataContainer;
		}
	...
	public interface DismissCallbacks<SomeCollectionView extends ViewAdapter> {
		...
		void onClick(SomeCollectionView recyclerView, int position);
	}
	...
	public Object makeScrollListener() {
		return mRecyclerView
				.makeScrollListener(new AbsListView.OnScrollListener() {
					@Override
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (rect.contains(x, y)) {
					assert (child instanceof ViewGroup && ((ViewGroup) child)
							.getChildCount() == 2) : "Each child needs to extend from ViewGroup and have two children";

					boolean dataContainerHasBeenDismissed = mPendingDismiss != null
							&& mPendingDismiss.position == mRecyclerView
			...
			if (mRowContainer != null) {
				...
				mDownPosition = mRecyclerView
						.getChildPosition(mRowContainer.container);
		...
		case MotionEvent.ACTION_CANCEL: {
			...
			if (mRowContainer != null && mSwiping) {
				// cancel
				mRowContainer.getCurrentSwipingView().animate().translationX(0)
		...
		case MotionEvent.ACTION_UP: {
			...
			if (clickDuration < 200) {
				...
				mCallbacks.onClick(mRecyclerView,
						mDownPosition);
			}
			...
			if (dismiss && mDownPosition != ListView.INVALID_POSITION) {
				// dismiss
				final RowContainer downView = mRowContainer; // mDownView gets
																// null'd before
																// animation
																// ends
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (mSwiping) {
				mRowContainer.getCurrentSwipingView().setTranslationX(
						deltaX - mSwipingSlop);
	...
	private void performDismiss(RowContainer dismissView, int dismissPosition) {
		...
		if (mPendingDismiss != null) {
			...
			int newPosition = mPendingDismiss.position < dismissPosition ? dismissPosition - 1
					: dismissPosition;
			processPendingDismisses();
	...
	public boolean undoPendingDismiss() {
		...
		if (existPendingDismisses) {
			...
			mPendingDismiss.rowContainer.dataContainer.animate()
	...
	private void processPendingDismisses(
			final PendingDismissData pendingDismissData) {
		mPendingDismiss = null;
		final ViewGroup.LayoutParams lp = pendingDismissData.rowContainer.container
				.getLayoutParams();
		final int originalHeight = pendingDismissData.rowContainer.container
				.getHeight();
		...
		animator.addListener(new AnimatorListenerAdapter() {
			...
			public void onAnimationEnd(Animator animation) {
				if (mCallbacks.canDismiss(pendingDismissData.position))
					mCallbacks.onDismiss(mRecyclerView,
							pendingDismissData.position);
				pendingDismissData.rowContainer.dataContainer
						.post(new Runnable() {
							@Override
							public void run() {
								pendingDismissData.rowContainer.dataContainer
								...
								pendingDismissData.rowContainer.dataContainer
										.setAlpha(1);
								pendingDismissData.rowContainer.undoContainer
								...
								pendingDismissData.rowContainer.container
										.setLayoutParams(lp);
							}
						});
			}
